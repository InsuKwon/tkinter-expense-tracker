...
class DashboardWindow(tk.Toplevel):
    ...
    def _build_overview_tab(self):
        ...
        # Recent activity frame - using tk.Frame with solid border instead of LabelFrame
        ...
        tk.Label(recent_container, text="Recent Activity",
                 font=('Arial', 12, 'bold'), bg='white').pack(anchor='w', padx=5, pady=(5, 0))
    ...
    def _update_recent_expenses(self):
        ...
        for date, cat, desc, amount in self.repo.get_recent_expenses(15):
            ...
            self.recent_tree.insert('', 'end', values=(date, cat, display_desc, f"${amount:.2f}"))
    ...
    def _update_charts(self, categories):
        ...
        # Chart 1: Category Pie Chart (limited to top 5 + Other)
        if categories:
            if len(categories) > 5:
        ...
        # Chart 3: Monthly Trends (sorted chronologically - oldest to newest)
        monthly_data = self.repo.get_monthly_spending()
        if monthly_data:
